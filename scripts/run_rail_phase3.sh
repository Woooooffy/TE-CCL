...
#   "coarse demand: N nonzero (U->V) pairs, distinct volumes [v]"  -- the rail cells are uniform,
#       so there must be exactly ONE distinct volume. More than one means coarsify_demand or the
#       cell declaration is wrong.
...
#   sbatch scripts/run_rail_phase3.sh alltoall     # alltoall (much heavier: 256x256 fine demand)
#
# Run locally instead (needs a Gurobi license on this box):
#   python -m teccl.examples.hierarchy_coarse_solve_rail allgather lp
# Add a 3rd arg `debug` to turn phase-3's per-cell narration back on (off here: 32 identical cells
# would each print a full derivation and bury the log).
...
srun python -m teccl.examples.hierarchy_coarse_solve_rail "${COLL}" lp
